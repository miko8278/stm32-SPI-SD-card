...
Ein wichtige, wenn auch in der DESIGN.md nur am Rande erw"ahnte Funktionalit"at, ist das Zur"ucklesen des
geschriebenen Buffers und ein Vergleichen der Nachricht. Damit kann littlefs feststellen, ob das 
Schreiben tats"achlich geklappt hat oder doch m"oglicherweise ein Schreibfehler vorliegt. Im Falle vom 
Flashspeicher k"onnte in diesem Falle ein Badblock vorliegen, also ein defekter Speicherblock.
Das Dateisystem probiert daher das Schreiben auf einem anderen Block und zwar solange, bis
...
Gleichzeitig best"atigt der Versuch, dass die zugrundeliegenden Blockschreibfunktionen "uber eine Vielzahl Aufrufe zuverl"assig arbeiten.
...
der Zeitpunkt erreicht, wo einmalig ein ganzer Zweig eines Baumes neu geschrieben werden muss.
...
um die Integrit"at der Daten und des Dateisystems zu sichern. Ganz trivial ist das allerdings nicht:
Wenn die Metadaten korrupt sind, so ist beispielsweise ein "Offnen der Datei mit der 
\texttt{fopen()}-Funktion von FatFs gar nicht m"oglich, da der Dateiname nicht stimmt. Viel einfacher scheint
hier die Einf"uhrung einer neuen, sicheren Schreibfunktion auf SD-Treiberebene,
...
Der SPI-Clock wurde auf 1 MHz belassen.
...
\fig{fig:nofstime} plottet die aufgenommen Daten. Dabei war bereits zu erwarten, dass das Schreiben
eines 512 Byte Blocks in etwa doppelt so lange in Anspruch nimmt als das Schreiben von zwei 512 Byte
Bl"ocken.
...
NoFs bei Nutzung der Einzelblockschreibfunktion. Es f"uhrt beim Abh"angen an eine Datei
...
\begin{table}[H]
    ...
    \end{tabular}
\end{table}    
Zeitm"a"sig deutlich abgehangen ist littlefs, wobei die Durchschnittsbildung in diesem Fall etwas irref"uhrend ist:
Grunds"atzlich dauert das Anh"angen von 1\,KB bei littlefs um die 80\,ms. Der Durchschnitt wird
aber durch fatale Ausrei"ser, die mehrere Sekunden dauern k"onnen, stark gesenkt. \\
...
Wartezeiten, weil die SD-Karte noch vorangegangene Operationen zuende f"uhren muss. \\ 
...
von Schreibfehler eine gezielte Injektion von falschen Daten erfolgen.\\ 
