...
Im Rahmen dieser Bachelorarbeit wurde die Verwendung von SD-Karten an einem Mikrocontroller untersucht.
...
SPI-Peripherien bei Mikrocontrollern sehr verbreitet sind.

Aufbauend auf der direkten Blockadressierung wurden anschließend verschiedene Dateisysteme betrachtet. 
Dazu wurden mit FatFs und littlefs zwei unterschiedliche Dateisysteme auf dem Mikrocontroller eingesetzt 
und an den zuvor entwickelten SD-Treiber angebunden. 
...
Ein Schwerpunkt der Arbeit lag auf der Untersuchung von Schreibfehlern
...
|           |-- fatfs_write_test.cpp      # Schreibtest 1+2 für FatFs
|           |-- littlefs_write_test.cpp   # Schreibtest 1+2 für littlefs
...
`-- test_data                             # Daten für die durchgeführten Schreitests
    |-- Writetest1                        # Versuch 1: Analyse der Schreibfehlerraten mittels bekannter Zeichenfolge
...
\begin{listing}[H]
    ...
    \end{minted}
\caption{Debugausgabe von \texttt{spi\_test.cpp} durch Anwendung von \texttt{spi\_test.gdb}}
\label{listing:gdbdebugausgabe}
\end{listing}

Eine typische Ausgabe in der GDB-Debug Konsole zeigt \coderef{listing:gdbdebugausgabe}.
...
Weil „D neu“ jetzt an einer ganz anderen Stelle auf dem Speichermedium liegt als das alte \enquote{D},
...
Im Grunde genommen werden nutzt littlefs genauso wie FatFs die Blockschreib- und Lesefunktionen des
...
eine ausf"uhrliche Erkl"arung des Prinzips in der
...
erl"autert. Diese ist "au"serst komplex und wird hier nur schemenhaft erl"autert:
Littlefs nutzt unterschiedliche Mechanismen f"ur Daten (den tats"achlichen Dateiinhalten) und Metadaten (Dateiname, Gr"o"se, etc.).
Der Mechanismus f"ur Daten ist eine modifizierte Form des bereits kurz erl"auterten COW, namentlich
das CObW (Copy-On-bounded-Write). Dieses f"uhrt einen Z"ahler ein, sodass nicht bei jeder Modifikation
der gesamte Baum neu geschrieben werden muss wie bei COW, sondern nur alle N male. Bis zur N-ten Modifikation
bleibt der alte Baum g"ultig. CObW wird bei littlefs also f"ur gr"o"sere Datenstrukturen verwendet, die 
viele Bl"ocke miteinander verketten m"ussen. F"ur die genutzte Verkettung 
wird eine sogenannte \enquote{CTZ skip-list} verwendet, ein von der Datenstruktur entfernter verwandter
einer verketteten Liste. Diese funktioniert zufallsbasiert statt deterministisch und hat eine Suchlaufzeit
von O(log n) f"ur einen speziellen Block, w"ahrend die Traversion durch alle Bl"ocke einer Datei in einer Laufzeit von O(n)
m"oglich ist und ein Anh"angen eines Blocks an eine Datei in O(1). Eine r"uckw"arts verkettete Liste h"atte beispielsweise den Nachteil, dass die Traversion durch
alle Bl"ocke O($n^2$) Schritte ben"otigt, w"ahrend eine vorw"arts verkettete Liste nicht an eine Datei Bl"ocke anf"ugen kann
ohne alle vorherigen Bl"ocke zu kopieren. Andere beliebte Datenstrukturen wie balancierte B"aume haben Nachteile
...
\begin{figure}[H]
    \centering
    \includegraphics[height=3cm]{Bilder_Diagramme/littlefsmetadata.png}
...
Die Metadaten stehen zu jedem Zeitpunkt in zwei Bl"ocken. Wenn die Metadaten neu geschrieben werden
m"ussen, wird der "altere der beiden Bl"ocke "uberschrieben und bei erfolgreichem Schreiben durch
eine neue Versionsnummer zum neuen Block erkl"art. Scheitert das Update so bleibt einfach der
alte Block g"ultig. Damit sind auch in heiklen F"allen die Metadaten einer Datei sicher. Zum
Vergleich: FatFs "uberschreibt seine Metadaten an Ort und Stelle und gef"ahrdet damit die
gesamte Dateiintegrit"at, falls die Schreiboperation scheitert. Zus"atzlich ist jeder Metablock
mit einer CRC32 abgesichert und nur ein g"ultiger Block wird akzeptiert.
Grunds"atzlich gilt f"ur Metadaten bei littlefs also der Begriff der Atomarität:
...
Berechtigungen existiert. Weiterhin sind Umlaute beim Dateinamen nicht erlaubt und
die maximale Dateinamenl"ange betr"agt 32 Zeichen. \\
An dieser Stelle stellt sich f"ur die Praxis die Frage, ob der Endnutzer mit dem Dateisystem
in Ber"uhrung kommen wird. W"ahrend die Einrichtung des littlefs FUSE wrapper Projektes  
f"ur technisch versierte und mit github vertraute Menschen problemlos in unter einer Stunde 
m"oglich ist, ist es f"ur normale Endnutzer ungeeignet und "uberfordernd. Littlefs ist
also kein Dateisystem, was f"ur Ger"ate wie beispielsweise Digitalkameras oder Oszilloskope
zu empfehlen w"are, wo dem Endnutzer eine garantierte Kompatiblit"at mit dem Dateisystem 
...
unterschiedlichen Optimierungsstufen. F"ur FatFs wurde eine kleine Standardardkonfiguration verwendet.
...
Hiervon ausgenommen ist FatFs, dessen statischer RAM-Verbrauch in optimierten Builds um fast 500 kB sinkt
...
W"ahrend das f"ur den verwendeten STM32-G431 mit 128 KB Flashspeicher kein Problem darstellt, so
...
\href{https://github.com/miko8278/stm32-SPI-SD-card/tree/main/test\_data/Writetest1}{test\_data/Writetest1/}.
Die Auswertung folgte am station"aren Desktop-Computer. Dazu wurden kurze Pythonskripte erstellt, die
Dateigr"o"se und Integrit"at der Daten pr"ufen. Das relevanteste Skript ist hierbei 
\href{https://github.com/miko8278/stm32-SPI-SD-card/blob/main/test\_data/Writetest1/test\_readable.py}{test\_data/Writetest1/test\_readable.py}.
...
W"ahrend in dieser Arbeit zwar vorrangig die Zuverl"assigkeit der Dateisysteme bez"uglich
...
der Dateierstellung wie in \fig{fig:versuch1loop} gemessen werden konnte.

\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{Bilder_Plot/1_fatfs_dateierstellzeit.png}
    \caption{Dauer der Neuerstellung von Dateien auf dem FatFs-Dateisystem}
    \label{fig:fatfstime}
\end{figure}

\fig{fig:fatfstime} zeigt die Dauer des Schreibvorgangs f"ur eine 1024 Byte Datei in Abh"angigkeit
davon wie viele Dateien bereits erstellt wurden. Es zeigt sich ein klare lineare Abh"angigkeit mit einem 
auff"allig periodisch wiederkehrendem Muster.

\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{Bilder_Plot/2_littlefs_dateierstellzeit.png}
    \caption{Dauer der Neuerstellung von Dateien auf dem littlefs-Dateisystem}
...
W"ahrend hier der "ubersichtlichkeithalber nur ein Auszug von rund 850 Dateien dargestellt ist, wurden alle vorhandenen
...
\fig{fig:dateierstellalle} best"atigt die Vermutung. Mit dem klassischen SFN-Format zeigt FatFs eine bessere Performance
beim Erstellen von Dateien als littlefs. 

\subsubsection*{Fazit Versuch 1}
Auch wenn in diesem Versuch keine Aussage "uber die Datenintegrit"at getroffen werden konnte, da die zugrundeliegende "Ubertragungsebene
...
Bei der Speicherung von Daten ist es f"ur die Geschwindigkeit des Schreibzyklus vorteilhaft diese in m"oglichst wenig Dateien zu b"undeln. Sollen dennoch
separate Dateien erzeugt werden, so ist es vorteilhaft diese ab einer gewissen Anzahl auf Unterordner aufzuteilen.
...
das der Performance, insbesondere da auf eingebetteten Systemen die Speicherbuffer sehr klein sind, mit denen gearbeitet wird.
...
Aufgrund der hohen Zuverl"assigkeit der Schreibfunktionen wird nun eine gezielte Fehlerinjektion 
in den Schreibfunktionen des SD-Kartentreibers durchgef"uhrt,
bei der nach jeweils 5000 Bytes ein Byte modifiziert wird. Da sowohl littlefs, als auch FatFs diese Funktion als 
...
Die aufgenommenen Rohdaten finden sich in \href{https://github.com/miko8278/stm32-SPI-SD-card/tree/main/test\_data/Writetest2}{test\_data/Writetest2/}.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{Bilder_Plot/3_WT2_littlefs_dateierstellzeit.png}
...
Durch Fehler sowohl beim Dateninhalt als auch in den Metadaten (also Dateiname, Zeitstempel, Dateigr"o"se etc.)
l"asst sich das zuvor verwendete Skript nicht zuverl"assig anwenden.


\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{Bilder_Plot/4_dateierstellzeit_all.png}
    \caption{Dauer der Neuerstellung von Dateien "Ubersicht}
...
war n"amlich nur bedingt m"oglich. Das zur"uckgebliebene inkonsistente Dateisystem hat verglichen mit 
...
Bei einem unsicheren "Ubertragungskanal f"ur die Daten bedarf es f"ur das FAT-Dateisystem eine Strategie um die
Integrit"at der Daten und des Dateisystems zu sichern.
Littlefs hingegen bringt durch die in Kapitel XXX besprochenen Schutzmechanismen hier klare Vorteile mit sich.
...
der Datei zu untersuchen. Weiterhin wurde der SPI-Takt auf 8 MHz bei diesem Versuch erh"oht,
um mehr potentielle Schreibfehler zu provozieren.\\
...
dass der Zeitaufwand der Neuerstellung einer Datei linear zunimmt bei
beiden Dateisystemen. F"ur die dauerhafte Aufnahme von Daten ist das sehr nachteilhaft:
F"ur eine Echtzeitgarantie bei der Aufnahme von Daten m"usste beachtet werden wie viele 
Dateien sich bereits im Ordner befinden und gegebenfalls m"usste ein neuer Ordner erstellt werden.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{Bilder_Plot/6_WT3_zeitenzoomin.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{Bilder_Plot/5_WT3_zeitenzoomout.png}
    \caption{Dauer des Anh"angens von 1\,KB an eine Datei in Abh"angigkeit der Dateigr"o"se}
...
Das weitaus fatalere Verhalten ist aber, dass das Anh"angen von Daten in regelm"a"sigen Abst"anden deutlich
...
Der vorhergehende Versuch war im Grunde genommen eine Vorbereitung f"ur ein 
gutes Design von diesem Versuch. Die Entwickler des littlefs-Dateisystems versprechen
...
Dieses greift allerdings zu kurz, da dabei keine Unterbrechung der SD-Karten-Spannungsversorgung
herbeigef"uhrt wird.
...
Die Spannungsversorgung der in dieser Bachelorarbeit verwendeten Platine wird entweder "uber die vorhandene 5V USB-C-Buchse
oder "uber eine Stiftleiste, die haupts"achlich f"ur
ein Batteriefach mit einer variablen Spannung zwischen 3,8 und 5,5 V gedacht ist, gew"ahrleistet. Welche der
...
Letztlich braucht es lediglich einen Schalter, der periodisch 5\,V an- und ausschaltet.
...
ein Strom von $\frac{5\,V}{13\,\Omega} \approx 385\,mA$ vom USB-C Zweig, andererseits ebenfalls maximal 385\,mA vom Zweig mit dem Kondendator $C_1$.
Da der MOSFET bis zu 5\,A schalten kann ist dies im sehr sicheren Bereich f"ur das Bauteil. \\
\begin{figure}[H]
    \centering
...
der \texttt{IRF620A} erst zuverl"assig bei $U_{gs}=5\,\mathrm{V}$ durch, sodass eine zus"atzlicher NPN-Transistor in Emitterschaltung
...
der Schreibfehler provozieren m"ochte, grunds"atzlich positiv zu bewerten ist. \\

Die verwendeten Programme f"ur den Test sind \texttt{test/fatfs\_killswitch\_test.cpp} und
\texttt{test/littlefs\_killswitch\_test.cpp}. Der Test an sich nutzt wieder eine f"ur
...
Das Programm pr"uft zun"achst wie viele Testdateien bereits existieren und erstellst
dann die n"achst gr"o"sere Datei.
...
praktisch keine Schreibfehler erzeugen lassen. Der Effekt war bei FatFs im "Ubrigen ebenfalls vorhanden.
...
\href{https://github.com/miko8278/stm32-SPI-SD-card/tree/main/test\_data/Writetest4/test\_readable.py}{test\_data/Writetest4/test\_readable.py}.
...
Der Versuch belegt also eine maximale Bitfehlerrate in der Gr"o"senordnung von $10^{-9}$ bei
exzessiven Stromausf"allen und fehlender CRC-Pr"ufsumme auf SD-"Ubertragungsebene.
...
Das Fazit ist hier, dass der zus"atzliche Datenschutz durch das littlefs-Dateisystem die h"ohere Komplexit"at
des Systems im Zusammenhang mit SD-Karten nicht rechtfertigt.

\subsection{Versuch 5: Direkte Blockadressierung (NoFs)}
...
Die Versuchsquelldateien findet sich in \texttt{test/nofs\_append\_write\_test\_512.cpp} und
...
f"ur zwei Bl"ocke (1024 Bytes). Das Schreiben von mehreren Bl"ocken nutzt einen unterschiedliches
SD-Kommando und sollte zeitm"a"sig dem Schreiben von einzelnen Bl"ocken "uberlegen sein.  
Auf einem Linuxsystem lassen sich die Rohdaten wiederrum mit \\
...
Auf der Softwareseite hat sich das Setup aus CMake, VsCode, Cortex-Debug und C++ bew"ahrt:
...
Die Arbeit am SD-Treiber f"ur blockweises Lesen und Schreiben von SD-Karten war eine
der Grundvoraussetzungen f"ur den Rest der Arbeit.
...
der "Ubertragung extrem niedrig sind. Durch Nutzung des SD-Karteninternen CRC-Pr"ufsumme besteht optional 
...
Schreiboperationen f"uhrt und damit potentiell die SD-Karte belastet. \\
...
Nachrichten"ubertragungskanal durchaus von Vorteil sein kann, wenn man die erw"ahnten Nachteile beachtet:
...
Redundanz l"asst sich ohne zus"atzlichen Rechenaufwand zus"atzliche Sicherheit schaffen mit dem Preis des Speicherplatzes.     
